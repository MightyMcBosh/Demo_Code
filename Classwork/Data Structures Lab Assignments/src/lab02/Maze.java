...
//		}
	 ...
	 */ 
	public String calculatePuzzlePassword(List<Tumbler> firstTumbler, List<Tumbler> secondTumbler, List<Tumbler> thirdTumbler) {
		
		for(int a = 0; a < firstTumbler.size(); a++) {
			for(int b = 0; b < secondTumbler.size(); b++) {
				for(int c = 0; c < thirdTumbler.size(); c++) {
					backpack.calculateTumblers(firstTumbler.get(a), secondTumbler.get(b), thirdTumbler.get(c)); 
											// nothing happens in "calculate tumblers!"
				}
			}
		}
	...
	private boolean binarySearch(Integer[] potions, Integer goal) {
		int low = 0, high = potions.length-1, mid = 50;
		while(low < high) {
			mid = (low + high) / 2;
